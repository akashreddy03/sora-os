...
use sora_os::{exit_qemu, gdt::DOUBLE_FAULT_IST_INDEX, hlt_loop, serial_println, serial_print, test_panic_handler};
use spin::LazyLock;
use x86_64::{VirtAddr, structures::{idt::{InterruptDescriptorTable, InterruptStackFrame}, paging::{FrameAllocator, Mapper, Page, PageTableFlags}}};
...
static IDT_TEST: LazyLock<InterruptDescriptorTable> = LazyLock::new(|| {
    ...
    unsafe { idt.double_fault.set_handler_fn(test_double_fault_handler).set_stack_index(DOUBLE_FAULT_IST_INDEX) };
    idt
});
...
pub extern "C" fn _start() -> ! {
    ...
    for page in Page::range_inclusive(stack_start_page, stack_end_page) {
        ...
        unsafe { mapper.map_to(page, frame, flags, &mut frame_allocator).expect("Mapping failed").flush(); };
    }    
    
    // Create a guard page by setting page table flags to 0
    unsafe {
        mapper.update_flags(stack_start_page, PageTableFlags::empty()).expect("Updating Flags Failed").flush();
    }
